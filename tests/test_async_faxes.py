...
@pytest.mark.anyio
async def test_list_sends_before_as_its_own_wire_param(
    respx_mock: respx.MockRouter, client: AsyncRingivo
) -> None:
    route = respx_mock.get(FAXES_URL).mock(return_value=httpx.Response(200, json={"data": []}))

    async with client:
        await client.faxes.list(before="0198c4a1-before")
...
@pytest.mark.anyio
async def test_list_reads_the_next_cursor_from_page_meta(
    respx_mock: respx.MockRouter, client: AsyncRingivo
) -> None:
